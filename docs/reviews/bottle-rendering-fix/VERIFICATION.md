...
- Outline state mapping is correct: the default outline shell is hidden=True; the valid-pour highlight switches to width 0.055 and color (1.00, 1.00, 1.00, 1.00); the sink-only state hides the outline shell (sink body looks like a regular bottle, only neck+dome bands are dark).
