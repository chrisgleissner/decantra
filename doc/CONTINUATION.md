...
- PlayMode tests failing: `ResetButton_RestoresInitialStateAndScore` in `Assets/Decantra/Tests/PlayMode/GameControllerPlayModeTests.cs`.
  - Failure message: `Expected: greater than 0 But was: 0` at line ~231 (assert on `ProvisionalScore > 0`).
  - Root cause: Level 1 can be generated already solved (optimal 0), so no valid moves / no provisional score. Need to prevent solved/optimal=0 levels in generator (ensure optimalMoves >= 2). Implemented new generator logic but tests not rerun due to user abort.
...
3. **If tests pass**:
   - Run PlayMode + EditMode tests (same script), then coverage gate `./tools/coverage_gate.sh`.
